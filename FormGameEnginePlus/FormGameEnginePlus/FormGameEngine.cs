...
public abstract class FormGameEngine
{
    ...
    public void DrawPixel(int x, int y, Pixel p)
    {
        if (x >= 0 && x < nScreenWidth && y >= 0 && y < nScreenHeight)
        {
            ...
            {
                if (p.a == 0) return;
                if (p.a == 255) pixelData[y * nScreenWidth + x] = p;
    ...
    public void Render(FormGameEngine fge)
    {
        ...
        if (fge.ScreenMousePos().InsideBounds(pos, pos + size))
        {
            ...
            if (Input.KeyHeld(MouseButtons.Left))
            {
                ...
                fge.DrawString(pos + size / 2f, text, textColor, textSize, PositionMode.Center);
            }
            else
            {
...
public class InputUI
{
    ...
    public void Render(FormGameEngine fge)
    {
        ...
        if (focused)
        {
            ...
            fge.DrawString((int)pos.x, (int)(pos.y + size.y / 2f), value, textColor, textSize, PositionMode.CenterY);
        }
        else
        {
            fge.FillRectangle(pos, size, idleColor);
